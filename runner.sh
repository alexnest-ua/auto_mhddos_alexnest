...
do	
	...
	then
		...
		then
			echo -e "\n[\033[1;32m$(date +"%d-%m-%Y %T")\033[1;0m] - \033[0;33m$threads is too LOW amount of threads for $proc_num CPUs - attack will be started with 4000 threads\033[0;0m\n"
			threads=4000
		fi
	
		if ((num_of_copies > 2));
		then 
			echo -e "\n[\033[1;32m$(date +"%d-%m-%Y %T")\033[1;0m] - \033[0;33mYou have only $proc_num CPU, so attack will be started only with 2 parallel attacks\033[0;0m\n"
			num_of_copies=2
		fi
	...
	then
		if ((threads < 5000));
		then
			echo -e "\n[\033[1;32m$(date +"%d-%m-%Y %T")\033[1;0m] - \033[0;33m$threads is too LOW amount of threads for $proc_num CPUs - attack will be started with 5000 async threads\033[0;0m\n"
			threads=5000
		fi
